...
_DATUM_OK = True               # wird in render() durch pruefe_datum gesetzt
...
def pruefe_datum(html, datum):
    """
    Datumspflicht nach Newsletter_Struktur.md, Abschnitt 8.

    Prüft drei Dinge.
        1. Kein Platzhalter ist unersetzt geblieben
        2. Das Ausgabedatum steht auf allen Seiten
        3. Kein Datum aus dem laufenden Jahr ist älter als sieben Tage

    Gibt True zurück, wenn nichts zu beanstanden ist.
    """
    kurz = datum.strftime("%d.%m.%Y")
    lang = f"{datum.day}. {MONATE[datum.month - 1]} {datum.year}"
    sauber = True

    offen = re.findall(r"__[A-Z0-9_]+__", html)
    if offen:
        print(f"  FEHLER  Unersetzte Platzhalter im Dokument: {', '.join(sorted(set(offen)))}")
        sauber = False

    treffer = html.count(kurz) + html.count(lang)
    if treffer < 6:
        print(f"  FEHLER  Das Ausgabedatum steht nur {treffer} mal im Dokument, erwartet mindestens 6.")
        sauber = False
    else:
        print(f"  Ausgabedatum {kurz} steht {treffer} mal im Dokument")

    # Alle Datumsangaben des laufenden Jahres einsammeln
    text = re.sub(r"<[^>]+>", " ", html)
    gefunden = set()
    for t, m, j in re.findall(r"\b(\d{1,2})\.\s?(\d{1,2})\.(\d{4})\b", text):
        gefunden.add((int(j), int(m), int(t)))
    monate_re = "|".join(MONATE)
    for t, mname, j in re.findall(rf"\b(\d{{1,2}})\.\s?({monate_re})\s+(\d{{4}})\b", text):
        gefunden.add((int(j), MONATE.index(mname) + 1, int(t)))

    veraltet = []
    for j, m, t in gefunden:
        try:
            d = datetime.date(j, m, t)
        except ValueError:
            continue
        if j == datum.year and 0 < (datum - d).days > 7:
            veraltet.append(d)

    if veraltet:
        liste = ", ".join(d.strftime("%d.%m.%Y") for d in sorted(veraltet))
        print(f"  PRÜFEN  Daten aus {datum.year}, die älter als 7 Tage sind: {liste}")
        print("          Jedes einzeln prüfen. Entweder korrigieren oder als Hintergrund bestätigen.")
    return sauber
...
def render(datum):
    ...
    global _DATUM_OK
    print("Datum")
    _DATUM_OK = pruefe_datum(html, datum)
...
def main():
    ...
    if nachrecherche:
        ...
        return
    if not (fill_ok and audit_ok and _DATUM_OK):
